...
const newsApiKey = require("./secret").newsApiKey;
...
mongoose.connect(newsConfig.uri).then(function() {
  Week.find({}, function (err, queries) {
    ...
    const modelDates = allOriginalDates.slice(0, -1); // last date only used for range
    if (modelDates.length !== 0) {
      createDateDocuments(modelDates, function() {
        // createImageFolders(modelDates);
...
function createDateDocuments(dates, cb) {
    let datesProcessed = 0;
    dates.forEach(function(date){
      createDateDocument(date, function() {
          datesProcessed++;
...
function createDateDocument(startDate, cb) {
